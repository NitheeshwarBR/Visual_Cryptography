  ...
  for (int i = 0; i < rows; ++i) {
    for (int j = 0; j < cols; ++j) {
      ...
      for (int k = 0; k < 3; ++k) {
        ...
        share1.at<Vec3b>(i, j)[k] = rand1 ? 255 : 0;
        share2.at<Vec3b>(i, j)[k] = rand2 ? 255 : 0;
        share3.at<Vec3b>(i, j)[k] = (rand1 ^ rand2) ? 255 : 0;
      }
    }
  }
}
...
Mat decrypt_shares(const Mat &share1, const Mat &share2, const Mat &share3) {
  ...
  Mat secret = Mat::zeros(rows, cols, CV_8UC3);

  for (int i = 0; i < rows; ++i) {
    for (int j = 0; j < cols; ++j) {
      for (int k = 0; k < 3; ++k) {
        int whitePixelCount = (share1.at<Vec3b>(i, j)[k] == 255) +
                              (share2.at<Vec3b>(i, j)[k] == 255) +
                              (share3.at<Vec3b>(i, j)[k] == 255);

        if (whitePixelCount == 3) {
          secret.at<Vec3b>(i, j)[k] = 255;
        } else {
          secret.at<Vec3b>(i, j)[k] = 0;
        }
      }
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  Mat share1 = Mat::zeros(Input_image.size(), CV_8UC3);
  Mat share2 = Mat::zeros(Input_image.size(), CV_8UC3);
  Mat share3 = Mat::zeros(Input_image.size(), CV_8UC3);
  ...
  Mat share1_read = imread("share1.png", IMREAD_COLOR);
  Mat share2_read = imread("share2.png", IMREAD_COLOR);
  Mat share3_read = imread("share3.png", IMREAD_COLOR);
